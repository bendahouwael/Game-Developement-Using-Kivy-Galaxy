...
class MainWidget(RelativeLayout):
    ...
    from users_actions import on_keyboard_up, on_keyboard_down, on_touch_down, on_touch_up, keyboard_closed
    menu_widget = ObjectProperty()
    perspective_point_x = NumericProperty(0)
    perspective_point_y = NumericProperty(0)
    V_NB_LINES = 8  # always odd number (can make it pair check doc)
    V_LINES_SPACING = .4  # percentage in screen width
    vertical_lines = []

    H_NB_LINES = 15  # always odd number (can make it pair check doc)
    ...
    def update_ship(self):
        ...
        x1, y1 = self.transform(*self.ship_coordinates[0])  # maghir * irah argument wahed w houwa hachtou 2
